...
function bind() {
  ...
  const debounced = debounce(() => doSearch().catch(alert), 400);
  ...
  const more = el('btnMore'); if (more) more.addEventListener('click', () => { state.limit += 50; if (el('limit')) el('limit').value = String(state.limit); doSearch().catch(alert); });
}
