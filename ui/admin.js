...
const toast = (msg, ms=2500) => { const n=el('toast'); if(!n){ alert(msg); return } n.textContent=msg; n.style.display='block'; clearTimeout(toast._t); toast._t=setTimeout(()=>{ n.style.display='none' }, ms) };
async function api(path, opts={}){ const res = await fetch(path, { headers:{'Content-Type':'application/json'}, credentials:'include', ...opts }); if(!res.ok){ let m=`${res.status} ${res.statusText}`; try{ const j=await res.json(); if(j.error) m=j.error }catch{}; throw new Error(m) } const ct=res.headers.get('content-type')||''; return ct.includes('application/json') ? res.json() : res.text() }
