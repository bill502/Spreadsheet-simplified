// JS migration: drop Status column from people, preserving all other columns/types/PK.
export default function(db){
  // Check if people exists and has Status column
  let info;
  try { info = db.prepare('PRAGMA table_info(people)').all(); } catch { return }
  if (!info || !info.length) return;
  const hasStatus = info.some(r => String(r.name) === 'Status');
  if (!hasStatus) return;

  const cols = info.filter(r => String(r.name) !== 'Status');
